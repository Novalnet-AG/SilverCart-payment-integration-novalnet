...
class NovalnetCallback extends DataObject {
    ...
    protected function handleInitialLevelPayments($nntransHistory, $callbacklogParams)
    {
        ...
        if (in_array($this->_captureParams['tid_status'], [99, 91]) && $nntransHistory->GatewayStatus == 75) {
            $callbackComments = PHP_EOL.'Novalnet callback received. The transaction status has been changed from pending to on hold for the TID:'. $this->_captureParams['shop_tid']. ' on '. date('Y-m-d H:i:s');
            ...
            $callbackComments = PHP_EOL. 'Novalnet callback received. The transaction has been confirmed on '. date('Y-m-d H:i:s');  
    ...
    protected function chargebackPayments($nntransHistory)
    {
        ...
        $chargebackComments = PHP_EOL.'Novalnet callback received. Chargeback executed successfully for the TID: ' . $this->_captureParams['tid_payment'] . ' amount ' . $formatedAmount . ' on ' . date('Y-m-d H:i:s') . '. The subsequent TID: ' . $this->_captureParams['tid'];

        $bookbackComments = PHP_EOL.'Novalnet callback received. Refund/Bookback executed successfully for the TID: '. $this->_captureParams['tid_payment']. ' amount ' . $formatedAmount . '. The subsequent TID: ' . $this->_captureParams['tid'];
    ...
    protected function transactionCancellation($nntransHistory)
    {
         if ($this->_captureParams['payment_type'] == 'TRANSACTION_CANCELLATION') {
             $transactionCancelComments = PHP_EOL.'Novalnet callback received. The transaction has been canceled on '.date('Y-m-d H:i:s');             
    ...
    protected function sendNotifyMail($data)
    {
        ...
        $enableEmail = NovalnetGateway::get()->filter('PaymentChannel', 'novalnetglobalconfiguration')->first()->EnableEmail;
        $content = $data['comments'];
        if ($enableEmail == 1 && Email::is_valid_address($recipient)) {
