...
class NovalnetGateway extends PaymentMethod
{
    ...
    private static $db = [
        ...
        'Cc3d'                          => 'Int',
        'PaymentAction' 				=> 'Enum("capture,authorize,default","default")',
        ...
        $this->beforeUpdateFieldLabels(function(&$labels) {
            $labels = array_merge(
                    ...
                    [
                        ...
                        'Gatewaytimeout'            => _t(self::class . '.Gatewaytimeout', 'Gateway timeout (in seconds)'),
                        'GatewaytimeoutDesc'        => _t(self::class . '.GatewaytimeoutDesc', 'Set the maximum wait time to receive a transaction response from the Novalnet system before marking the transaction as incomplete. The recommended limit is 240 seconds'),
                        'ReferrerID'                => _t(self::class . '.ReferrerID', 'Referrer ID'),
                        'ReferrerIDDesc'            => _t(self::class . '.ReferrerIDDesc', 'Referrer ID of the person/company who recommended Novalnet to you'),
                        'ManualTestingVendorScript' => _t(self::class . '.ManualTestingVendorScript', 'Allow manual testing of vendor script process'),
                        'ManualTestingVendorScriptDesc' => _t(self::class . '.ManualTestingVendorScriptDesc', 'Enable to test Novalnet vendor script manually. Disable before setting your shop live to block unauthorized calls from external parties. Refer the Novalnet Payment Bundle Installation Guide for further details'),
                        'EnableEmail'               => _t(self::class . '.EnableEmail', 'Enable e-mail notification'),
                        'EnableEmailDesc'           => _t(self::class . '.EnableEmailDesc', 'Enable this option to notify the given e-mail address when the vendor script is executed successfully. Refer the Novalnet Payment Bundle Installation Guide for further details'),
                        'SendTo'                    => _t(self::class . '.SendTo', 'Send e-mail to'),
                        'SendToDesc'                => _t(self::class . '.SendToDesc', 'Vendor script execution messages will be sent to this e-mail'),
                        'VendorScriptConfiguration' => _t(self::class . '.VendorScriptConfiguration', 'Vendor Script Management'),
                        'VencorConfiguration'       => _t(self::class . '.VencorConfiguration', 'Novalnet Global Configuration'),
                        'OnholdStatusManagement'    => _t(self::class . '.OnholdStatusManagement', 'Order status management for on-hold transaction(-s)'),
                        'OnholdOrderStatus'         => _t(self::class . '.OnholdOrderStatus', 'On hold order status'),
                        'OnholdOrderStatusDesc'     => _t(self::class . '.OnholdOrderStatusDesc', 'Status to be used for on-hold orders until the transaction is confirmed or canceled'),
                        'OnholdOrderCancelStatus'   => _t(self::class . '.OnholdOrderCancelStatus', 'Order cancellation status'),
                        'OnholdOrderCancelStatusDesc'   => _t(self::class . '.OnholdOrderCancelStatusDesc', 'Status to be used when order is canceled or fully refunded'),
                        'OrderConfirmationSubmitButtonTitle'    => _t(self::class . '.OrderConfirmationSubmitButtonTitle', 'Pay with Novalnet (over 100 payment methods worldwide)'),'novalnetcreditcardConfiguration'   => _t(self::class . '.novalnetcreditcardConfiguration', 'Credit Card Configuration'),                       
                        'Creditcard3d'                          => _t(self::class . '.Creditcard3d', 'Enable 3D secure'),
                        'Creditcard3dDesc'                      => _t(self::class . '.Creditcard3dDesc', '3D secure adds a layer of authentication for credit card transactions where the buyer is required to enter a password on their bank&#39;s interface. This reduces the risk of charge-backs.'),
                        ...
                        'InvoiceDueDate'               => _t(self::class .'InvoiceDueDate','Payment due date (in days)'),
                        'InvoiceDueDateDesc'           => _t(self::class .'InvoiceDueDateDesc','Number of days given to the buyer to transfer the amount to Novalnet (must be greater than 7 days). In case this field is empty, 14 days will be set as due date by default'),                        
                        'GuaranteePaymentPendingStatus' => _t(self::class .'GuaranteePaymentPendingStatus','Pending payment order status'),
                        ...
                        'CallbackStatus'        => _t(self::class .'CallbackStatus','Callback order status'),
                        ...
                        'OrderCompletionStatus' => _t(self::class .'OrderCompletionStatus', 'Order completion status'),
                        'OrderCompletionStatusDesc' => _t(self::class .'OrderCompletionStatusDesc', 'Status to be used for successful orders'),
                        'novalnetcashpaymentConfiguration'   => _t(self::class . '.novalnetcashpaymentConfiguration', 'Barzahlen Configuration'),                       
                        ...
                        'StatusCancelled'             => _t(self::class .'StatusCancelled','Canceled'),
                        'MinimumOrderAmount'          => _t(self::class . '.MinimumOrderAmount', 'Minimum transaction amount for authorization'),
                        'MinimumOrderAmountDesc'       => _t(self::class . '.MinimumOrderAmountDesc', 'Transactions above this amount will be "authorized only" until you capture. Leave blank to apply to all transactions.'),
                        'Default'     => _t(self::class . '.Default', 'None'),
    ...
    protected function getFieldsForNnGlobalConfig($fields) : void
    {
        ...
        $filedDetails = ['VendorId' => 'VendorIdDesc', 'AuthCode' => 'AuthCodeDesc', 'ProductId' => 'ProductIdDesc', 'TariffId' => 'TariffIdDesc', 'AccessKey' => 'AccessKeyDesc', 'GatewayTimeout' => 'GatewaytimeoutDesc', 'ReferrerID' => 'ReferrerIDDesc'];
    ...
    protected function getFieldsForNnVendorScriptConfig($fields) : void
    {
        ...
        $filedDetails = ['ManualTestingVendorScript' => 'ManualTestingVendorScriptDesc', 'EnableEmail' => 'EnableEmailDesc'];
    ...
    protected function getFieldsForNnPaymentConfiguation($fields) : void
    {
        ...
        $paymentAction  = [
                'default' => $this->fieldLabel('Default'),
        ...
        if ($this->PaymentChannel == 'novalnetinvoice') {
            $tabBasicFieldList->push(TextField::create('InvoiceDueDate',$this->fieldLabel('InvoiceDueDate'))
                    ->setDescription($this->fieldLabel('InvoiceDueDateDesc'))
            );
        }

        if ($this->PaymentChannel == 'novalnetcreditcard') {
            $tabBasicFieldList->push(CheckboxField::create('Cc3d', $this->owner->fieldLabel('Creditcard3d'))
                        ->setDescription($this->fieldLabel('Creditcard3dDesc'))
            );           
        }
    ...
    public function requireDefaultRecords() : void
    {
        parent::requireDefaultRecords();
         ...
         $requiredStatus = [
            'cancelled'  => $this->fieldLabel('StatusCancelled'),
        ...
        if ($novalnetPayments->exists()) {			
            foreach ($novalnetPayments as $novalnetPayment) {
				...
				$novalnetPayment->OnholdOrderCancelStatus = PaymentStatus::get()->filter('Code', 'cancelled')->first()->ID;   				
    ...
    {
        $referrerID = NovalnetGateway::get()->filter('PaymentChannel', 'novalnetglobalconfiguration')->first()->ReferrerID;
        ...
        $paygateParams['tariff']    = NovalnetGateway::get()->filter('PaymentChannel', 'novalnetglobalconfiguration')->first()->TariffId;
        $paygateParams['referrer_id'] = !empty($referrerID) ? $referrerID : '';
        $paygateParams['test_mode'] = ($this->mode == 'Dev') ? '1' : '0';               
    }
    ...
    public function sendServerRequest($paymentData, $url)
    {
        ...
        curl_setopt($ch, CURLOPT_TIMEOUT, ($GatewayTimeout != '' ? $GatewayTimeout : 240));
